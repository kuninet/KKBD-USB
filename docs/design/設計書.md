...
2. `config_init()` — GPIO初期化・ジャンパーピン読み取り
3. `led_init()` — LED GPIO初期化・点灯（起動表示）
4. `uart_out_init(config_get_baudrate())` — UART初期化
5. `usb_host_init()` — TinyUSB ホスト初期化（`tuh_init(BOARD_TUH_RHPORT)` のラッパ）
6. `keyrepeat_init()` — キーリピート状態初期化
7. メインループ開始
...
- `config`、`led`、`uart_out`、`usb_host`、`keyrepeat`
...
| 機能 | GPIO | 方向 | 設定 | 備考 |
|------|------|------|------|------|
| UART0 TX | GPIO 0 | 出力 | UART機能 | SBCのRXへ接続。TTLレベル3.3V出力 |
| JP1（行末コード bit0） | GPIO 10 | 入力 | プルアップ | OPEN=High=1 / SHORT=Low=0 |
| JP2（行末コード bit1） | GPIO 11 | 入力 | プルアップ | OPEN=High=1 / SHORT=Low=0 |
| JP3（ボーレート bit0） | GPIO 12 | 入力 | プルアップ | OPEN=High=1 / SHORT=Low=0 |
| JP4（ボーレート bit1） | GPIO 13 | 入力 | プルアップ | OPEN=High=1 / SHORT=Low=0 |
| LED | GPIO 25 | 出力 | デジタル出力 | Pico内蔵LED（PICO_DEFAULT_LED_PIN） |
| USB | 内蔵USB | - | USBホスト | Micro-BコネクタをUSBホストとして使用 |

> 注意: GPIO 0 は UART0 TX の代替機能として使用する（`gpio_set_function(0, GPIO_FUNC_UART)`）。UART0 RX（GPIO 1）は使用しない（UARTは送信専用）。
...
*本文書は KKBD-USB プロジェクトの詳細設計書（バージョン 1.0）です。実装の進捗に応じて内容を更新・精緻化します。*
