...
- [ ] **条件 1**: USB キーボードで A〜Z キーを押すと、小文字 a〜z が UART に送信される（TC-401 合格）
- [ ] **条件 2**: 0〜9 キーを押すと、数字 0〜9 が UART に送信される（TC-402 合格）
- [ ] **条件 3**: 未対応キー（F キー、矢印キー等）を押しても UART への誤送信が発生しない（TC-404 合格）
- [ ] **条件 4**: キーを押し続けても、離して再度押さない限り同じ文字が繰り返し送信されない（TC-405 合格）
- [ ] **条件 5**: 同じキーを押し続けた場合の差分検出ロジックが正しく動作する（TC-405 合格）
- [ ] **条件 6**: ホスト側ユニットテスト（`KKBD_PHASE4_DONE=ON`）の 4 関数（`test_keymap_convert_lowercase`, `test_keymap_convert_digits`, `test_keymap_convert_unsupported`, `test_keymap_is_enter`）すべてがパスする
...
| ボーレート | （例: 9600） |
| 行末コードジャンパー設定 | （例: JP1=OPEN, JP2=OPEN → CR） |

### TC 別結果

| TC | 操作 | 期待動作 | 結果 | 備考 |
|----|------|---------|------|------|
| TC-401 | `a`〜`z` 入力 | `abcdefghijklmnopqrstuvwxyz` 表示 | | |
| TC-402 | `0`〜`9` 入力 | `0123456789` 表示 | | |
| TC-403 | Enter キー押下 | 行末コード送信（ジャンパー設定に従う） | | |
| TC-404 | F1/F2/矢印キー等押下 | シリアル端末に何も表示されない | | |
| TC-405 | `a` を 3 秒押し続け | `a` が 1 文字のみ表示される | | |
| TC-406 | Shift/Ctrl 単独押下 | シリアル端末に何も表示されない | | |
| TC-407 | テンキー Enter 押下 | 行末コード送信（メイン Enter と同じ） | | |
| TC-408 | `hello`+Enter+`world`+Enter | `hello`+行末コード+`world`+行末コード | | |

### ユニットテスト結果

| テスト関数 | 結果 | 備考 |
|-----------|------|------|
| `test_keymap_convert_lowercase` | | |
| `test_keymap_convert_digits` | | |
| `test_keymap_convert_unsupported` | | |
| `test_keymap_is_enter` | | |
