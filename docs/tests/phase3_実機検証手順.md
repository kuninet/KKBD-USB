...
- [x] **条件 1**: USB キーボードを接続すると、シリアル端末に接続ログが表示される（TC-302 合格）
- [x] **条件 2**: USB キーボードを抜くと、シリアル端末に切断ログが表示される（TC-303 合格）
- [x] **条件 3**: キーボード未接続でも Pico がクラッシュせず、接続待ち状態を維持する（TC-301 合格）
...
| シリアル端末ソフト | （macOS 標準）|
| 使用キーボード | 複数 HID インターフェース搭載のキーボード（Boot Keyboard + メディアキー等の Generic HID）|
...
| TC | 操作 | 期待ログ・動作 | 結果 | 備考 |
|----|------|--------------|------|------|
| TC-301 | 起動時にキーボード未接続 | Pico がクラッシュせず接続待ち状態を維持 | ☑ 合格 | 起動バナー後、安定動作 |
| TC-302 | USB キーボード接続 | `[USB] Keyboard connected (addr=N, instance=M)` | ☑ 合格 | `addr=1, instance=0` で正常検出 |
| TC-303 | USB キーボード切断 | `[USB] Keyboard disconnected` | ☑ 合格 | 切断ログ正常 |
| TC-304 | 抜き差しを 5 回繰り返し | 毎回 connected / disconnected ログが正常に表示 | ☑ 合格 | 4 サイクル以上の抜き差しで毎回正常再認識 |
| TC-305（任意） | USB マウス接続 | Keyboard connected ログが出ない（`proto=0` または `proto=2` で無視ログが出る） | ☑ 合格（自動検証） | 使用キーボードが複数 HID インターフェースを持ち、Boot Keyboard 以外（メディアキー等の Generic HID, `proto=0`）が `[USB] Non-keyboard HID ignored (proto=0)` として正常に無視されることを実機で確認 |
...
| 受け入れ条件 3（TC-301） | ☑ 合格 |
| **Phase 3 総合** | ☑ **合格** |
| 所感・備考 | 実機ログ：起動時に `[USB] TinyUSB host initialized` → 起動バナー → キーボード接続で `[USB] Keyboard connected (addr=1, instance=0)` と同時に複数 HID インターフェースの非キーボードを `[USB] Non-keyboard HID ignored (proto=0)` で正しく無視。切断時 `[USB] Keyboard disconnected` を確認。4 サイクルの抜き差しすべて正常。TC-305 が実機キーボードのマルチインターフェース構成により自動的に検証された。|
| 添付（写真・動画） | - |
