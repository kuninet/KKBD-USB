...
- [x] **条件 1**: ジャンパーなし（全 OPEN = 9600 bps）で起動し、PC のシリアル端末に `KKBD-USB Ready` が表示される（TC-201 合格）
- [x] **条件 2**: JP3/JP4 の各パターン（4 種類）を変えて再起動し、それぞれのボーレートで文字列が受信できる（TC-201〜TC-204 全合格）
- [x] **条件 3**: 予約パターン（JP1/JP2 両方 SHORT）で起動した際、CR 相当のデフォルト動作となる（TC-205 合格）
...
| シリアル端末ソフト | （macOS 標準: `screen` または同等）|

### TC 別結果

| TC | ジャンパー設定 | 期待ボーレート | 結果 | 備考 |
|----|--------------|--------------|------|------|
| TC-201 | 全 OPEN | 9600 bps | ☑ 合格 | `KKBD-USB Ready` が 1 秒毎に正常受信 |
| TC-202 | JP3=SHORT, 他 OPEN | 19200 bps | ☑ 合格 | 同上 |
| TC-203 | JP4=SHORT, 他 OPEN | 38400 bps | ☑ 合格 | 同上 |
| TC-204 | JP3=SHORT, JP4=SHORT | 115200 bps | ☑ 合格 | 同上 |
| TC-205 | JP1=SHORT, JP2=SHORT | 9600 bps（CR フォールバック）| ☑ 合格 | 予約パターンでもクラッシュせず正常起動・送信 |
...
|------|------|
| 受け入れ条件 1（TC-201） | ☑ 合格 |
| 受け入れ条件 2（TC-201〜TC-204） | ☑ 合格 |
| 受け入れ条件 3（TC-205） | ☑ 合格 |
| **Phase 2 総合** | ☑ **合格** |
| 所感・備考 | `scripts/build.sh --clean` でビルド成功。USB-シリアル変換アダプタ経由で 4 種ボーレート（9600/19200/38400/115200）すべてで `KKBD-USB Ready` 受信を確認。予約パターン（JP1/JP2 両 SHORT）でもクラッシュせず正常動作。 |
| 添付（写真・動画） | - |
