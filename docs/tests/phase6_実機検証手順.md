...
- [x] **条件 1**: JP1/JP2 が OPEN/OPEN の場合、Enter で CR（0x0D）が送信される（TC-607）
- [x] **条件 2**: JP1 が SHORT の場合、Enter で LF（0x0A）が送信される（TC-608）
- [x] **条件 3**: JP2 が SHORT の場合、Enter で CRLF（0x0D 0x0A）が送信される（TC-609）
- [x] **条件 4**: キーを押し続けると、約 500ms 後から約 50ms 間隔でキーリピートが動作する（TC-604）
- [x] **条件 5**: キーボード未接続時に LED が点灯（待ち受け状態）する（TC-601、本実装では低速点滅）
- [x] **条件 6**: キーボード接続時に LED が点灯（接続済み状態）する（TC-602）
- [x] **条件 7**: UART 送信のたびに LED が短時間点滅する（TC-603）
...
| シリアル端末ソフト | `python3 -m serial.tools.miniterm -f hexlify` で行末コード確認 |
| 使用キーボード | JIS（106/109）配列キーボード |
| USB OTG アダプタ / ケーブル | VBUS 給電パススルー対応 |
| ボーレート | 9600 |
| 行末コードジャンパー設定 | TC-607〜609 で 3 パターン全確認 |

### TC 別結果

| TC | 操作 | 期待動作 | 結果 | 備考 |
|----|------|---------|------|------|
| TC-601 | キーボード未接続で起動 | BOOT 点灯 → 低速点滅切替 | ☑ 合格 | LED 状態遷移正常 |
| TC-602 | キーボード接続 | LED 常時点灯（MOUNTED） | ☑ 合格 | 接続検出と同時に MOUNTED |
| TC-603 | キー押下時の LED | 短時間点灯維持→復帰 | ☑ 合格 | 連続入力時の点灯維持確認 |
| TC-604 | `a` を 3 秒押し続ける | 約 500ms 後リピート開始、約 50ms 間隔で連投 | ☑ 合格 | リピート速度・初回遅延正常 |
| TC-605 | リピート中にキーを離す | リピート即座に停止 | ☑ 合格 | 全キーリリース検出正常 |
| TC-606 | `a` 押しっぱなし中に `b` 追加押下 | `b` が 1 文字送信、`a` のリピート停止、`b` が 500ms 遅延後 50ms 間隔でリピート開始 | ☑ 合格 | リピート対象置換正常 |
| TC-607 | JP1=OPEN, JP2=OPEN で Enter | 0x0D（CR）のみ送信 | ☑ 合格 | hexlify で確認 |
| TC-608 | JP1=SHORT, JP2=OPEN で Enter | 0x0A（LF）のみ送信 | ☑ 合格 | hexlify で確認 |
| TC-609 | JP1=OPEN, JP2=SHORT で Enter | 0x0D 0x0A（CRLF）送信 | ☑ 合格 | hexlify で確認 |
| TC-610 | キーボード切断 | WAIT_DEVICE に戻り低速点滅再開 | ☑ 合格 | umount コールバック正常 |
| TC-611 | Enter キー長押し | 1 回のみ行末コード送信 | ☑ 合格 | Enter リピート対象外を確認 |
| TC-612（任意） | ERROR 状態の意図的再現 | コードレビューで状態遷移確認 | - | 任意項目、未実施 |

### ユニットテスト結果

| テスト関数 | 結果 | 備考 |
|-----------|------|------|
| `test_keyrepeat` 全 6 関数 | ☑ 合格 | `keyrepeat_decide` 純粋関数テスト全パス |
...
| 受け入れ条件 7（TC-603） | ☑ 合格 |
| **Phase 6 総合** | ☑ **合格** |
| 所感・備考 | キーリピート（500ms/50ms）と行末コード（CR/LF/CRLF）が実機で正常動作を確認。LED 5 状態（BOOT/WAIT/MOUNTED/TX/ERROR）も期待通りに遷移。Phase 4 までで実装した行末コード送信パスが Phase 6 で本格運用可能になり、メイン機能がほぼ完成。 |
| 添付（写真・動画） | - |
